...
export interface ProjectDetailViewProps extends ProjectDetailProps {
  /** When set, skips theme lookup from `project.accent`. */
...
export function ProjectDetailView({
  project,
  onDismiss,
  resolvedAccent: resolvedAccentProp,
}: ProjectDetailViewProps) {
  const shouldReduceMotion = useReducedMotion();
  const sectionVariants = useOrderedRouteSectionVariants();
  const resolvedFromData = useResolvedProjectAccent(project);
  const resolvedAccent = resolvedAccentProp ?? resolvedFromData;
